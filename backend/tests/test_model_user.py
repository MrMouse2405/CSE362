...
class TestUserDefaults:
    ...
    def test_role_defaults_to_student(self):
        user = User(email="alice@example.com", hashed_password="hash")  # type: ignore
...
class TestUserRoleAssignment:
    ...
    def test_create_with_enum_role(self, role: UserRole):
        user = User(email="bob@example.com", hashed_password="hash", role=role)  # type: ignore
...
class TestUserRoleValidation:
    ...
    @pytest.mark.parametrize(
        "bad_role", ["moderator", "superadmin", "", "STUDENT", "Admin"]
    )
